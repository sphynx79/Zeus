...
class Report < Mongodb
  ...
  class << self
    ...
    def get_report(start_dt, end_dt, type)
      collection = case type
                  when "centrali_tecnologia_daily"
                    client[:remit_centrali_daily_tecologia].aggregate(pipeline_daily_tecologia_no_cache(start_dt, end_dt)).allow_disk_use(true).to_a
                  when "centrali_zona_daily"
                    client[:remit_centrali_daily_zona].aggregate(pipeline_daily_zona_no_cache(start_dt, end_dt)).allow_disk_use(true).to_a
                  when "centrali_tecnologia_hourly"
                    client[:remit_centrali_hourly_tecologia].aggregate(pipeline_hourly_tecologia_no_cache(start_dt, end_dt)).allow_disk_use(true).to_a
                  when "centrali_zona_hourly"
                    client[:remit_centrali_hourly_zona].aggregate(pipeline_hourly_zona_no_cache(start_dt, end_dt)).allow_disk_use(true).to_a
                  else
                    "Errore"
                  end
      collection
    ...
    def pipeline_daily_tecologia_no_cache(start_dt, end_dt)
      pipeline = []
    ...
    def pipeline_daily_zona_no_cache(start_dt, end_dt)
      pipeline = []
    ...
    def pipeline_hourly_tecologia_no_cache(start_dt, end_dt)
      pipeline = []
    ...
    def pipeline_hourly_zona_no_cache(start_dt, end_dt)
      pipeline = []
      ...
      pipeline << {
        "$project": {
          ...
          data: {"$dateToString": {
            ...
            # timezone: "Europe/Rome",
            # dentro il db è gia in timezone italiano 30-08-2018 00:00:00
            # quando ho creato la collection per questo report, ho già raggruppato le ore 
            # del giorno con il timezone italiano vedere il programma che scarica le remit centrali
          }},
          brnn: "$brnn",
          cnor: "$cnor",
